...
public class CoachScreenController implements Initializable{
    ...
    public void openAddCoachWindow(){
        Helpers.openWindow("/org/example/add_coach.fxml", "Add a New Coach");
    }
    public void openCoachEditor() {
        Coach c = coachTable.getSelectionModel().getSelectedItem();
        if (c == null) return;

        EditCoachController ctrl = (EditCoachController)Helpers.openWindow("/org/example/edit_coach.fxml", "Assign Classes to a Coach").getController();
